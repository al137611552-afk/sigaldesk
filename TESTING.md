...
4. 截图组通过的话，打开 docs\design\shots\ 看刚生成的那批图，
   报告有没有：文字被遮挡、元素重叠、明显留白、数字显示成 0 或乱码。
   **重点看这两张**（都是新加的）：
     · panel-15-预警组.png —— 九个标的同屏。每格头部应当是一行放下
       「标的名 · 最新价 · 当日涨跌」，不该换行；没坐满的格子写「等待信号」
     · panel-11-九宫格.png —— 同一标的九个周期。买卖点徽章应当**贴在 K 线的
       价格上**（不是浮在 K 线上方/下方）

5. 顺手验一条今天新加的保护：连开两个盯盘进程应当被拒绝。
   在包目录下开两个窗口，各跑一次
     .venv\Scripts\python.exe scripts\watch.py --crypto-only --minutes 2
   第二个应当打印「已有一个盯盘进程在用同一个运行态」并以退出码 2 退出。
   （这一条要联网取加密行情，OKX 公开接口不需要凭据；不方便就跳过并说明。）

不要做：不要填任何凭据，不要开实盘交易
（config\trading.yaml 的 enabled 必须保持 false）。
第 1~4 步全程离线；只有第 5 步会联网，且只连 OKX 公开行情。
```
...
冒烟还会验这些（都不需要真行情）：买卖点画在 `priceToCoordinate(触发价)` 上、
密集处折成 ×N、预警组的钉住/未读/空槽、两种网格模式切换时格子摘干净。
