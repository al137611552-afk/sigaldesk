...
- **十字线只在九周期模式联动**（`crosshairSyncs()`）：同一标的看各级别同一时刻才有意义；
  预警组是九个不同品种，联动只是晃眼。`shift+单击` 锁定同刻同理只在联动模式下有效。
- **顶部读数按模式给**：单图/九周期是同一标的，有意义（九周期取 1m 那格的最新一根）；
  预警组九格九个品种，清空。**别一律清空** —— 那会把九周期那份也清掉（踩过）。
...
## 统计与评估
- **标准误必须按持仓重叠折算**（`effective_n`）：持有期长于冷却期时信号不独立，
  同一段行情被重复计入。1m 扳机的变体名义 n=250、有效 n 只有 7 —— 不折算的话
  会把噪声当成强证据（真发生过，我据此给过一次方向完全相反的结论）。
- **试规则变体用 `rule_eval.py --rules-dir`**，别改 `config/rules/` ——
  那是盯盘进程正在用的。忘了恢复就会拿改过的配置报数（踩过）。
